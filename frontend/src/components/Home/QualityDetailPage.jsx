...
// แปลง link ของ item ให้เป็น URL ที่ใช้งานได้
function resolveLink(item) {
  ...
  return getFileUrl(link)
}

// จัดกลุ่มรายการตาม category
...
function DiseaseCard({ item }) {
  ...
  // ถ้าไม่มี link แสดงเป็น Card ปกติ
  return (
    <div className="group">
      {cardInner}
    </div>
...
export function QualityDetailPage({ quality, onBack }) {
  ...
  if (!quality) {
    return (
      <div className="mx-auto max-w-[900px] px-8 py-16 text-center">
        ...
        <button
          onClick={onBack}
          className="mx-auto mt-3 flex items-center gap-1.5 rounded-lg border-none bg-transparent text-[13px] font-medium text-blue-600"
        >
  ...
  const articleItems = Array.isArray(quality.articleItems)
    ? quality.articleItems
    : []
          ...
          <>
            ...
            {articleItems.some((item) => item.img) && (
              <div className="mb-5 flex flex-col gap-6">
                ...
                {groupByCategory(
                  articleItems.filter((item) => item.img)
                ).map((group) => (
                  ...
                  >
                    ...
                    {group.category && (
                      <div className="flex items-center gap-2 border-b border-line pb-1.5">
                        ...
                        <h4 className="text-[13.5px] font-bold text-navy-900">
                          {group.category}
                        </h4>
